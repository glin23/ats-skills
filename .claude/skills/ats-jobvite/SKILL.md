...
1. **Chrome with CDP 9222 已启动**（lily Profile）：
   ...
   ```
2. **`shared/profile.json` 存在**（schema 见 `shared/profile.template.json`）+ `resume_path` 指向本地 PDF。
...
- **JS widget 模式** — 极少数 tenant 把 JobVite 当 widget 嵌到自己网站，DOM 是 iframe 或动态注入；helper 抓不到 → 提示 Lee 用对应公司官网走 manual。
...
PLAN=$(node shared/cdp.mjs eval "$TAB" "JSON.stringify(JobVite.fillForm($PROFILE))")
```
...
- **`action: "manual"`** → 截图 + 列出未知字段问 Lee，log 出来下次完善 helpers。
...
- **JS widget tenant** — 有公司把 JobVite 当 widget 嵌官网；这种情况 helper 抓不到 → 退出让 Lee manual。
...
| `detectExternalRedirect().redirected === true` | 报告 target_url，让 Lee 决定（可能是 widget tenant），退出 |
| `detectApplyForm` 始终 `ok: false` | 截图 + 让 Lee 检查页面 / 是否登录态问题 |
| `typetext` 后 value 没进去 | 重试 1 次；仍失败 → 截图让 Lee 手填 |
| 文件上传 fail | 重试 1 次；仍失败 → 截图让 Lee 手传 |
| `fillForm` plan 含 `manual` 项 | 截图 + 列出未知字段问 Lee |
| Submit 后 `checkSuccess.ok === false` | 截图 `*_jobvite_post_submit_fail.png`，留页面让 Lee 看；可能 form 验证失败回到 error state |
| JobVite 显示 rate limit / 403 | 报错退出，提醒 Lee 改天再投 |
...
- 不要在 fail 时改 helpers.js 静默推进 — 先 raise 给 Lee
...
- v0.8 dogfood log（待 Lee 第一次跑后填）：`log/jobvite_dogfood_2026-XX.md`
