...
```
告诉 Lee："已填完 + 简历已上传，截图 `/tmp/ashby_pre_submit.png`。看一眼，确认无误后说『投』我才点 submit。"
...
- `_systemfield_name` 是 **combined** "Lee Lin"，不分 first/last
...
| `Ashby.fillForm` 返回 plan 里有 `needs_manual_answer` | 截图 + 列出未知字段问 Lee 怎么填 |
| `typetext` 完毕后 value 没进去 | 重试一次；仍失败 → 截图 + 报告 Lee 手填 |
| `clickYesNo` 返回 `click_did_not_activate` | DOM 结构变了，截图 + 报告 |
| `pickSelect` 返回 `option_not_found` | 选项文本对不上，列出实际选项问 Lee |
| Submit 后 `checkSuccess.ok = false` | 截图 + 留页面给 Lee 看（可能是 validation error 没填全） |
