...
3. **简历 PDF 可读**：`profile.resume_path`（用户 默认 `/Users/lee/Desktop/用户_Lin_Resume.pdf`）。**先 `cp` 到 `/tmp/`** — Lever 的 drag-drop 区在 macOS 沙盒外的路径上会触发误报（见 Known gotchas #5）。
...
- the user's ATS 标准答案库（Notion）：page `35d1e8ce818581e697a9fb4bfd36f250`
