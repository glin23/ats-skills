...
**先检查 URL** — 如果重定向到 `career.sap.com` 或 `careers.successfactors.com`，立刻停下并告诉 Lee 该公司已迁到 SF，本 skill 不处理。
...
### 3. 注入 helpers + 先看实际表单
**v0.8 beta 关键**：不要直接信任 `fillForm.plan`。先 dump 一次 required fields 给 Lee 看：
```bash
...
```
看返回的 label/selector/type — 如果和 profile.json 字段对得上就继续；如果有大量 unknown 字段或 selector 看起来不对，截图给 Lee，先停下手动诊断。
...
- **`action: "select"`** → native `<select>`：
  ...
  ```
  如果返回 `not_native_select`，说明是 react-select — 这版没实现，截图问 Lee 手填或先 patch helpers。
...
```
告诉 Lee："已填完 + 简历已上传，截图 `/tmp/smartrecruiters_pre_submit.png`。**这是 v0.8 beta 第一次跑这家**，先确认无误后说『投』我才点 submit。"
...
| 症状 | 处理 |
|---|---|
| URL 跳到 SAP / SuccessFactors | 停下，告诉 Lee 走手投或等 ats-successfactors |
| `cdp.mjs goto` 404 / 超时 | URL 死了，报告 Lee + 退出（不写 log） |
| `findEmptyRequired` 返回里 label 大量 `null` 或 `(no label)` | DOM 跟预期不一致，截图 + 停下诊断 |
| `fillForm.plan` 为空 | 选择器没匹配上，截图给 Lee + 让 Lee 报 field name |
| `typetext` 完毕后 value 没进去 | 重试一次；仍失败 → 截图 + Lee 手填 |
| `pickSelect` 返回 `not_native_select` | react-select — v0.8 未支持，需 patch helpers |
| `pickSelect` 返回 `option_not_found` | 选项文本对不上，列出 options 问 Lee |
| Submit 后 `checkSuccess.ok = false` | 截图 + 留页面给 Lee 看（可能 validation 没过） |
