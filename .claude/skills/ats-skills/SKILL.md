...
- Notion DB / data_source IDs come from `~/.ats-skills/config.json` (see `shared/config.template.json`). For 用户's legacy setup the defaults in `shared/paths.mjs` apply.
