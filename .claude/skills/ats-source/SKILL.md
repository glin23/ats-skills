...
1. **只 source + score，不 filter**：`min_fit_score` 是 用户 在 Notion dashboard 里筛 view 用的，不是这里硬剔。打 3 分的也要写进 Notion，让 用户 自己决定。
2. **Skip-not-fail**：单家公司 API 404 / 单条 job AI 评分 throw / 单 row Notion upsert 错 → log + 继续下一项，不中断 batch。
3. **De-dupe via Notion**：跟现有 「📋 岗位追踪」 DB 里同 URL 的 row 合并 — `upsertJob()` 已经按 Apply URL 找现有 row，存在则 PATCH（保留 用户 手工 edit 的状态）；不存在则 CREATE 新 row 默认 `🤖 AI sourced`。
4. **Feedback loop inject**：每次 sourcing 都 load 最近 20 个 skip feedback 喂进 AI scorer system prompt，让模型避免重复推荐 用户 已经 skip 过的同类 role。
...
- ❌ **不要在 Step 4 用 `replace` 删旧 row** — 用 `upsertJob` 按 URL 合并，保留 用户 手工 edit 的 状态 / Bot 备注。
- ❌ **不要 git commit / push** — 这是 runtime skill，不是 build pipeline。
- ❌ **不要 retry 整个 batch** — 单 job/row 错 skip-not-fail；整个 batch 跑一次结束，dashboard 显示 errors，用户 决定要不要再跑。
...
- `shared/sourcing/company_list.json` — 用户 维护的高 fit 公司种子列表（PR 扩展）
