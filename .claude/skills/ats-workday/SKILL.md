...
工作流是先 parse URL 拿 tenant subdomain → 查 `companies/<slug>.json` → 不存在就报错让 用户 先做 config → 存在就按 step_definitions 一步步填。
...
test -f "$(jq -r '.resume_path // "/Users/lee/Desktop/用户_Lin_Resume.pdf"' shared/profile.json)" || { echo "ERROR: resume PDF missing"; exit 1; }
...
If Workday demands account creation/signin, 用户 handles it manually in the visible Chrome window (do NOT script credentials).
...
{ "ok": true, "step": {...}, "detectedStep": {...}, "plan": [
  {"action": "typetext", "selector": "[data-automation-id='legalName--firstName']", "value": "用户"},
  {"action": "select",   "autoId": "addressSection_countryRegion", "value": "California"},
  {"action": "upload",   "selector": "[data-automation-id='file-upload-input-ref']", "value": "/Users/lee/Desktop/用户_Lin_Resume.pdf"},
...
- **`action: "missing"`** → required field has no profile value. STOP, screenshot, ask 用户.
...
- Many tenants force account creation. 用户 handles login manually; do NOT script credentials.
...
| Symptom | Action |
|---|---|
| `No config for <slug>` | Tell 用户 to follow "How to add a new company" |
| `applyCompanyConfig` returns `no_matching_step_in_config` | Config step numbering / names don't match `detectStep()`. Screenshot + ask 用户 to inspect |
| `plan[]` contains `action: "missing"` | Required profile field unset. Screenshot + ask 用户 to update profile.json or skip company |
| `typetext` value not retained | Try once more; on second failure screenshot + report (some tenants use stricter trust checks) |
| Account creation required | Tell 用户, hand control to the visible Chrome window |
| `clickContinue` returns `continue_button_not_found` | DOM layout changed. Screenshot, fall back to manual click |
| `checkSuccess.ok = false` after submit | Don't write success log. Screenshot + ask 用户 to verify in browser |
