...
- **反爬强（CloudFront + 部分 Akamai Bot Manager）** — 严格用 Lee 的真 Chrome session；**严禁 headless / fresh-profile / proxy**。建议 **≤3 投递/天/tenant** 直到摸到 throttle 边界。
...
|---|---|
| `true` | 截图 + 报告 Lee："iCIMS 要求登录 `<tenant>` 账户，请在 Chrome 里完成登录后 reply '已登录'。" 暂停。 |
| `false` | 继续。 |
...
```
让 Lee 看截图决定怎么填（推荐 decline to answer），手填完后 reply "继续"。
...
- **每 tenant 独立账号** — 跨 tenant 不复用 session；每个 `careers-X.icims.com` 都要 Lee 单独注册一次。
...
| `detectLoginRequired().loginRequired === true` | 暂停 + 提示 Lee 注册/登录 `<tenant>` 账户 |
| `fillForm` plan 里有 `manual` 项 | 截图 + 列出未知字段问 Lee |
| `typetext` 后 value 没进去 | 重试 1 次；仍失败 → 截图让 Lee 手填 |
| 文件上传 fail | 重试 1 次；仍失败 → 截图让 Lee 手传 |
| 多步 wizard 第 N 步 Next 失败 | 截图 + 报错让 Lee 看是哪个字段没过校验 |
| Submit 后 `checkSuccess.ok === false` | 截图 `*_icims_post_submit_fail.png`，留页面让 Lee 看 |
| iCIMS 显示 "Daily limit" / 403 | 报错退出，提醒 Lee 改天再投 + 检查反爬触发 |
...
- v0.8 dogfood log（待 Lee 第一次跑后填）：`log/icims_dogfood_2026-XX.md`
