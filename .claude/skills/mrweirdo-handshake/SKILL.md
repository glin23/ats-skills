...
1. **Chrome with CDP 9222 已启动**，使用 Lee 的 lily Profile（保证 Handshake 已登录）：
   ```bash
...
- **document picker 默认 "most recent resume"** — 大多数情况这是对的（Lee 的 resume），但如果 Handshake account 里有多版本要明确指定 `profile.document_preferences.resume`。
- **TLS / cookie 反爬** — Handshake 是普通 Cloudflare 站，没有 LinkedIn 级行为分析。但用 Lee 的真 Chrome session 还是最稳。**严禁 headless / fresh-profile / proxy。**
...
|---|---|
| `true` | 截图 + 报告 Lee："请在 Chrome 里登录 Handshake，登录完成后 reply '已登录' 我继续。" 暂停。 |
| `false` | 继续。 |
| `null` (unknown) | 截图给 Lee 让他判断。 |

| `detectRedirectToExternalATS().redirected` | 处理 |
|---|---|
| `false` | 继续走本 skill 后续步骤。 |
| `true` + target_ats=`greenhouse` | 报告 Lee："Handshake 外跳到 Greenhouse (`<target_url>`) — 建议跑 `/mrweirdo-greenhouse <target_url>`。" 退出。 |
| `true` + target_ats=`ashby` | 同上，dispatch 到 `/mrweirdo-ashby`。 |
| `true` + target_ats=`workday`/`lever`/`icims`/`unknown` | 报告 + 退出，Lee 决定。v0.6 暂不支持 workday auto-dispatch。 |
...
- **`action: "typetext"`** → 走 CDP `typetext`（保 isTrusted=true，与 Ashby 同）：
  ...
  ```
- **`action: "pick_document"`** + `note: "verify_default_selection"` → 截图确认默认选中的是 Lee 最新简历就跳过；不对再调 `Handshake.pickDocument(name, type)`。
...
```
列出剩余空字段 → 让 Lee 确认是否要补 / profile.json 加字段 / 直接授权 submit。
...
- **登录态依赖用户 Chrome session** — Handshake 走学校 SSO，automation 无法登录。Lee 必须事先在 lily profile 登好。
...
| `detectLoginRequired().loginRequired === true` | 暂停 + 提示 Lee 登录 + 等"已登录"信号 |
| `detectRedirectToExternalATS().redirected === true` | 报告 target_url + target_ats，让 Lee 跑对应 sub-skill，退出 |
| `Handshake.fillForm` plan 里有 `needs_manual_answer` | 截图 + 列出未知字段问 Lee |
| `typetext` 后 value 没进去 | 重试 1 次；仍失败 → 截图让 Lee 手填 |
| Submit 后 `checkSuccess.ok === false` | 截图 `*_handshake_post_submit_fail.png`，留页面让 Lee 看；可能是 multi-step wizard 后面还有 page |
| Handshake 显示 "Daily limit reached" (300) | 报错退出，提醒 Lee 改天再投 |
...
- 不要在 fail 时改 helpers.js 静默推进 — 先 raise 给 Lee，由他决定怎么改
...
- v0.6 dogfood log（待 Lee 第一次跑后填）：`log/handshake_dogfood_2026-XX.md`
